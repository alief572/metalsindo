				...
				<?php if (empty($results)) {
					...
					foreach ($results as $record) {
						...
						<tr>
							...
							<td><?= number_format($record->inventory_idr) ?></td>
							<td><?= number_format($record->uang_muka_idr) ?></td>
							<td><?= number_format($record->selisih_kurs) ?></td>
							<td><?= number_format($record->unbilled_idr) ?></td>
							<td style="padding-left:20px">
								<a class="btn btn-info btn-sm" href="<?= base_url('/incoming/request/' . $record->id_data) ?>" title="Request Pembayaran AP"><i class="fa fa-edit"></i></a>
							</td>
...
<script type="text/javascript">
	...
	$(document).on('click', '.view3', function() {
		...
		var pp = 'JV005';
		var akses = 'jurnalunbilled';
		// alert(id);
		$("#head_title").html("<i class='fa fa-list-alt'></i><b>Detail Jurnal</b>");
		$.ajax({
			type: 'POST',
			url: siteurl + 'jurnal_nomor/jurnal_fn/jurnal_unbilled/' + id + '/' + pp + '/' + akses,
			data: {
				'id': id
			},
			success: function(data) {
