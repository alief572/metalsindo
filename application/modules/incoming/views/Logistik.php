...
$po	= $this->db->query("SELECT * FROM dt_incoming WHERE id_data = '".$head->id_data."' limit 1 ")->row();

$no_po	= $this->db->query("SELECT * FROM dt_trans_po WHERE id_dt_po = '".$po->id_dt_po."' limit 1 ")->row();

$loi	= $this->db->query("SELECT * FROM tr_purchase_order WHERE no_po = '".$no_po->no_po."' ")->row();

$paymentpib	= $this->db->query("SELECT * FROM tr_pib_payment")->result();
 ...
 <div class="box box-primary">
    <div class="box-body">
		<form id="data-form" method="post">
			...
			<div class="row">
		<center><label for="customer" ><h3>Input Biaya Export/Import Handling</h3></label></center>
				<div class="col-sm-12">
		<div class="col-sm-6">
		<div class="form-group row">
			...
			<div class="col-md-8">
				...
				<input type="hidden" id="id_data" name="id_data" class="form-control" value="<?=$head->id_data ?>">
			</div>
		</div>
		</div>
		<div class="col-sm-6">
		...
		<div class="col-sm-6">
			...
			<div class="col-md-4">
				<input type="text" id="loi" name="loi" class="form-control sm" value="<?= $loi->loi ?>" readonly>
			</div>
			</div>
		</div>
		...
		<div class="form-group row">
				<div class="col-md-4">
					<label for="id_customer">Pilih PIB</label>
				</div>
				<div class="col-md-4">
				<select name="pib" id="pib" class="form-control sm select2"> 
				<?php foreach($paymentpib as $pib){ ?>
				<option value="<?=$pib->kd_pembayaran?>"><?= strtoupper($pib->kd_pembayaran).'|'.$pib->keterangan?></option>
				<?php } ?>				
				</select>
		...
		<div class="col-sm-6">
			<div class="form-group row">
				<div class="col-md-4">
					<label for="id_customer">Pilih Pengiriman</label>
				</div>
				<div class="col-md-4">
				<select name="pengiriman" id="pengiriman" class="form-control sm select2"> 				
		...
		<div class="col-sm-6">
			...
			<div class="col-md-4">
				<input type="text" id="kurs" name="kurs" class="form-control sm divide" readonly>
			</div>
			</div>
		</div>
		<div class="col-sm-6">
		<div class="form-group row">
			...
			<div class="col-md-4">
				<input type="hidden" id="biaya" name="biaya" class="form-control sm divide">
			</div>
		</div>
		</div>
		...
		<div class="form-group row" >
			...
			<thead>
				<tr class='bg-blue'>
					...
					<th width='7%'>Total Berat <br> Receive</th>
					<th width='7%' hidden>Lot. No</th>
					<th width='7%' hidden>Panjang</th>
					<th width='10%'>Gudang</th>
					<th width='7%' hidden>Berat Aktual</th>
					<th width='7%' hidden>Selisih</th>
				</tr>
			</thead>
		...
		<tr id='trmaterial_$loop'>
			...
			<td class='text-right'>
			<input  type='text' 		value='".$material->total_berat."' class='form-control' id='width_recive_".$loop."' 		required name='dt[".$loop."][width_recive]' readonly>			
			</td>			
...
<script type="text/javascript">
						...
						$.ajax({
							...
							success		: function(data){								
								if(data.status == 1){											
									...
									window.location.href = base_url + active_controller+'/index_logistik';
								}else{
	...
	$(document).on('change', '#pib', function(){
		var pib=$("#pib").val();
		$.ajax({
            type:"GET",
            url:siteurl+'incoming/get_pib',
            data:"pib="+pib,
            success:function(data){
               $("#kurs").val(data);
            }
