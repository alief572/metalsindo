<?php
$ENABLE_ADD     = has_permission('Approval_PO_Non_Material.Add');
$ENABLE_MANAGE  = has_permission('Approval_PO_Non_Material.Manage');
$ENABLE_VIEW    = has_permission('Approval_PO_Non_Material.View');
$ENABLE_DELETE  = has_permission('Approval_PO_Non_Material.Delete');
?>
			...
			<div class="col-md-8">
				<input type="date" class="form-control" id="expect_tanggal" required name="expect_tanggal"  >
									...
									<div class="col-md-8">
										<select id="supplier" name="supplier" class='form-control input-md chosen-select' disabled>
											...
											<?php foreach ($results['list_supplier'] as $supplier) {
												$selected = '';
												if (isset($results['header_po']) && $supplier->id_suplier == $results['header_po']->id_suplier) {
													$selected = 'selected';
												}
											?>
									...
									<table id="example" class='table table-bordered table-striped'>
										...
										<tbody>
											...
											foreach ($results['getitemso'] as $value) {
												...
												$get_trans_po = $this->db->get_where('dt_trans_po_non_material', ['idpr' => $value->id])->num_rows();
												...
												$header_po = $this->db->get_where('dt_trans_po_non_material', ['idpr' => $value->idpr])->row();
					...
					<div class="col-sm-12">
						<div class="col-sm-6">
							<div class="form-group row">
								...
								<div class="col-md-7" id="ForHarga">
									<input type="text" class="form-control" id="persenppn" onkeyup required name="persenppn" onblur="cariTotal()" placeholder="Persen PPN (%)" value="<?= $results['header_po']->total_ppn_persen ?>" readonly>
									<input type="text" class="form-control" id="totalppn" onkeyup required name="totalppn" onblur="cariTotal()" placeholder="Nilai PPN" value="<?= $results['header_po']->total_ppn ?>" readonly>
								</div>
							</div>
						</div>
					</div>
						...
						<div class="col-sm-6">
							<div class="form-group row">
								...
								<div class="col-md-8" id="ForSum">
									<input readonly type="text" class="form-control" id="subtotal" value="<?= number_format($total_harga_ppn + $results['header_po']->total_ppn + $results['header_po']->taxtotal, 2) ?>" onkeyup required name="subtotal">
								</div>
							</div>
						</div>
						<div class="col-sm-6">
					...
					<center>
						...
						<a href="<?= base_url('approval_po_non_material') ?>" class="btn btn-sm btn-danger"><i class="fa fa-arrow-left"></i> Back</a>
					</center>
				</div>
			</div>
		</form>
				...
				swal({
					...
					function(isConfirm) {
						if (isConfirm) {
							...
							var baseurl = siteurl + active_controller + '/approve_po_process';
							$.ajax({
								url: baseurl,
								type: "POST",
								data: formData,
				...
				swal({
					...
					function(isConfirm) {
						if (isConfirm) {
							...
							var formData = new FormData($('#data-form')[0]);
							var baseurl = siteurl + active_controller + '/reject_po_process';
							$.ajax({
								url: baseurl,
								type: "POST",
								data: formData,
