...
class Pembayaran extends Admin_Controller
{
	...
	protected $viewPermission   = "Pembayaran.View";
	protected $addPermission    = "Pembayaran.Add";
	...
	public function __construct()
	{
		...
		$this->template->title('Pembayaran Invoice');
	...
	public function index()
	{
		...
		$data = $this->pembayaran_model->get_data_pn();
		$this->template->set('results', $data);
		$this->template->title('Indeks Of Payable');
		$this->template->render('list_payment');
	}

	public function index_uang_muka()
	{
		$this->template->page_icon('fa fa-list');
		$data = $this->pembayaran_model->get_data_request();
		$this->template->set('results', $data);
		$this->template->title('Indeks Of Payable');
		$this->template->render('list_payment_uangmuka');
	}

	public function pembayaran_pib()
	{
		$this->template->page_icon('fa fa-list');
		$data = $this->pembayaran_model->get_data_pn_pib();
		$this->template->set('results', $data);
		$this->template->title('Indeks Of PIB Payable');
		$this->template->render('list_payment_pib');
	}
	...
	public function index_jurnal_um()
	{
		...
		$this->template->set('results', $data);
		$this->template->title('Indeks Of Payment');
		$this->template->render('index_jurnal_uangmuka');
	}

	public function index_jurnal_pib()
	{
		$this->template->page_icon('fa fa-list');
		$data = $this->pembayaran_model->get_data_request_payment_pib();
		$this->template->set('results', $data);
		$this->template->title('Indeks Of Payment PIB');
		$this->template->render('index_jurnal_pib');
	}


	public function pembayaran_buktipotong($kd_bayar)
	{
	...
	{
		$this->pembayaran_model->get_data_json_inv();
	}
	...
	{
		$this->pembayaran_model->get_data_json_payment();
	}
	...
	{
		$this->pembayaran_model->modal_detail_invoice($this->uri->segment(3));
	}
	public function modal_detail_request()
	{
		$this->pembayaran_model->modal_detail_request($this->uri->segment(3));
	}
	public function modal_detail_pib()
	{
		$this->pembayaran_model->modal_detail_pib($this->uri->segment(3));
	}
	...
	public function save_pembayaran()
	{
		...
		$kd_bayar 			= $this->pembayaran_model->generate_nopn($Tgl_Invoice);
		...
		$idcustomer = $this->input->post('supplier');

		$customer =  $this->db->query("SELECT * FROM master_supplier WHERE id_suplier = '$idcustomer'")->row();

		$idcs   = $idcustomer;
		$nmcs	= $customer->name_suplier;
		...
		for ($i = 0; $i < count($this->input->post('kode_produk')); $i++) {
			...
			$this->db->insert('tr_request_payment_detail', $datadetail);
			//Update QTY_AVL
			$invoice = $this->input->post('kode_produk')[$i];

			$jmlbyr  = str_replace(",", "", $this->input->post('jml_bayar')[$i]);
			$Qry_Update	 = "UPDATE tr_request SET total_bayar_idr=total_bayar_idr + $jmlbyr, sisa_invoice_idr=sisa_invoice_idr - $jmlbyr WHERE id='$invoice'";
			$this->db->query($Qry_Update);


			$so  = $this->db->query("SELECT * FROM tr_request WHERE id='$invoice'")->row();
			$no_so = $so->no_po;

			$Qry_Update_so	 = "UPDATE tr_incoming SET total_bayar_ap=total_bayar_ap + $jmlbyr WHERE id_data='$no_so'";
			$this->db->query($Qry_Update_so);


			if ($matauang == 'usd') {
				$jmlbyrrupiah  += str_replace(",", "", $this->input->post('jml_bayar')[$i]) * $kurs;
			} else {
				$jmlbyrrupiah  = 0;
			}
		}

		$bankrupiah = str_replace(",", "", $this->input->post('total_bank')) * $kurs;
		$selisih    = $bankrupiah - $jmlbyrrupiah;
		...
		$this->db->insert('tr_request_payment', $data);
		...
		if ($tambah_lebih_bayar != 0) {
			...
			$data_lebih_bayar[]			= array(
				...
				'keterangan'         => $nmcs,
				'totalpembayaran'    => str_replace(",", "", $this->input->post('tambah_lebih_bayar')),
	...
	function appr_jurnal()
	{
		...
		$this->print_pembayaran_fix();
	}


	function print_pembayaran_fix()
	{
		...
		$show = $this->load->view('pembayaran/print_pembayaran', $data, TRUE);
		...
		$header = '
          ...
          <table border="0" width="100%">
            <tr><b>
                  <td width="15%" style="font-size:8pt !important;vertical-align:top"><b>Kode Pembayaran</b></td>
	...
	public function unlocated()
	{
		...
		$this->template->title('Pembayaran Unlocated');
	...
	public function lebihbayar()
	{
		...
		$datacoa  	     = $this->Acc_model->GetCoaCombo();
		$template  	     = $this->Acc_model->GetTemplate();
		$this->template->title('Pembayaran Lebih Bayar');
	...
	public function save_unlocated()
	{
		...
		for ($i = 0; $i < count($this->input->post('keterangan')); $i++) {
			$datadetail = array(
				...
				'bank'              => $this->input->post('bank'),
				'totalpembayaran'   => $this->input->post('totalpembayaran')[$i],
				'saldo'             => $this->input->post('totalpembayaran')[$i],
				'created_on'    => date('Y-m-d H:i:s'),
				'created_by'    => $session['id_user']
			);
			...
			$jumlah_total    = $this->input->post('totalpembayaran')[$i];
	...
	public function save_lebihbayar()
	{
		...
		for ($i = 0; $i < count($this->input->post('tanggal')); $i++) {
			$datadetail = array(
				...
				'saldo'             => $this->input->post('totalpembayaran'),
				'created_on'    => date('Y-m-d H:i:s'),
				'created_by'    => $session['id_user']
			);
	...
	public function jurnal_bum()
	{
		...
		$session = $this->session->userdata('app_session');
		$this->template->page_icon('fa fa-list');
		$data = $this->pembayaran_model->get_data_pn_jurnal();
		$this->template->set('results', $data);
		$this->template->title('Jurnal Pembayaran');
		$this->template->render('index_jurnal_pembayaran');
	}


	public function save_pembayaran_uangmuka()
	{
		...
		$data_session 	    = $this->session->userdata;
		$kd_bayar 			= $this->pembayaran_model->generate_nopn_uangmuka($Tgl_Invoice);
		...
		// print_r($kd_bank);
		// exit;
		if ($this->input->post('matauang') == 'usd') {
			$kurs = $this->input->post('kurs');
		} else {
			$kurs = 1;
		}
		...
		$data = array(
			...
			'kurs_bayar' => $kurs,
			'jumlah_piutang' => str_replace(",", "", $this->input->post('request')),
			'jumlah_piutang_idr' => str_replace(",", "", $this->input->post('request')) * $kurs,
			'jumlah_bank' => str_replace(",", "", $this->input->post('total_bank')),
			'jumlah_bank_idr' => str_replace(",", "", $this->input->post('total_bank')) * $kurs,
			'jumlah_pembayaran' => str_replace(",", "", $this->input->post('total_terima')),
			'jumlah_pembayaran_idr' => str_replace(",", "", $this->input->post('total_terima')) * $kurs,
			...
			'biaya_admin' => str_replace(",", "", $this->input->post('biaya_adm')),
			'biaya_admin_idr' => str_replace(",", "", $this->input->post('biaya_adm')) * $kurs,
			'biaya_ppn' => str_replace(",", "", $this->input->post('ppn')),
			'biaya_ppn_idr' => str_replace(",", "", $this->input->post('ppn')) * $kurs,
			'created_by'    => $session['id_user'],
			'created_on' => date('Y-m-d H:i:s'),
			'jenis_pph' => $this->input->post('jenis_pph'),
			'no_account' => '-',
		...
		$uangmuka    = 	str_replace(",", "", $this->input->post('request'));
		$uangmukaidr =	str_replace(",", "", $this->input->post('request')) * $kurs;
		$totalbayar  =	str_replace(",", "", $this->input->post('total_bank'));
		$bayar       =	str_replace(",", "", $this->input->post('total_bank')) * $kurs;
		$no_po       =  $this->input->post('no_po');

		$invoice = $this->input->post('no_invoice');

		$this->db->insert('tr_request_payment_uangmuka', $data);

		$Qry_Update	 = "UPDATE tr_request SET total_bayar_kurs=total_bayar_kurs + $totalbayar,total_bayar_idr=total_bayar_idr + $bayar, sisa_invoice_kurs=sisa_invoice_kurs - $totalbayar, sisa_invoice_idr=sisa_invoice_idr - $bayar, status_close='1' WHERE no_request='$invoice'";
		$this->db->query($Qry_Update);


		$Qry_Update_so	 = "UPDATE tr_purchase_order SET uang_muka_kurs=uang_muka_kurs + $uangmuka,uang_muka_idr=uang_muka_idr + $uangmukaidr, sisa_um_kurs=sisa_um_kurs + $uangmuka, sisa_um_idr=sisa_um_idr + $uangmukaidr, kurs_uangmuka=$kurs WHERE no_po='$no_po'";
		$this->db->query($Qry_Update_so);
	...
	public function save_pembayaran_pib()
	{
		...
		$data_session 	    = $this->session->userdata;
		$kd_bayar 			= $this->pembayaran_model->generate_nopb($Tgl_Invoice);
		...
		$data = array(
			...
			'jumlah_bank' => str_replace(",", "", $this->input->post('total_bank')),
			'jumlah_bank_idr' => str_replace(",", "", $this->input->post('total_bank')) * $kurs,
			'bea_masuk' => str_replace(",", "", $this->input->post('beamasuk')),
			'bea_masuk_idr' => str_replace(",", "", $this->input->post('beamasuk')) * $kurs,
			'kd_bank' => $kd_bank,
			'vat_in' => str_replace(",", "", $this->input->post('beamasuk')),
			'vat_in_idr' => str_replace(",", "", $this->input->post('beamasuk')) * $kurs,
			'biaya_pph' => str_replace(",", "", $this->input->post('pph')),
			'biaya_pph_idr' => str_replace(",", "", $this->input->post('pph')) * $kurs,
			'created_by'    => $session['id_user'],
			'created_on' => date('Y-m-d H:i:s'),
			'jenis_pph' => $this->input->post('jenis_pph'),
			'no_account' => '-',
		...
		$this->db->insert('tr_pib_payment', $data);
