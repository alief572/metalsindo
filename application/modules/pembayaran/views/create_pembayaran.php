<?php
$inv          =  $this->db->query("SELECT * FROM tr_request WHERE no_request = '$no_inv' ")->row();
?>
...
<form id="form-header-mutasi" method="post">
						...
						<div class="col-sm-6 form-horizontal">
							<div class="row">
								<div class="form-group">
									<label for="ket_bayar" class="col-sm-4 control-label">Nomor Request </font></label>
									<div class="col-sm-6">
										<input type="text" name="no_invoice" id="no_invoice" class="form-control input-sm" value="<?= $inv->no_request ?>" readonly>
										<input type="hidden" name="no_po" id="no_po" class="form-control input-sm" value="<?= $inv->no_po ?>" readonly>
							...
							<div class="row">
								<div class="form-group ">
									...
									$tglinv = date('Y-m-d');
									?>
									<label for="tgl_terima_invoice" class="col-sm-4 control-label">Tgl Terima Invoice :</label>
									<div class="col-sm-6">
										<input type="date" name="tgl_terima_invoice" id="tgl_terima_invoice" class="form-control input-sm tanggal" value="<?php echo date('Y-m-d', strtotime($inv->tgl_terima_invoice)) ?>" readonly>
									</div>
								</div>
							</div>
							<div class="row">
							...
							<div class="form-group">
								<label for="jenis_invoice" class="col-sm-4 control-label">PPN </font></label>
								<div class="col-sm-6">
									<input type="text" name="ppn" id="ppn" class="form-control input-sm" value="<?= number_format($inv->nilai_ppn) ?>">
								</div>
							</div>
							<div class="form-group ">
								<label for="tgldo" class="col-sm-4 control-label">Kurs</label>
								<div class="col-sm-6">
									<input type="text" name="kurs" class="form-control input-sm" id="kurs">
								</div>
							</div>
						</div>
						<div class="col-sm-6 form-horizontal">
							<div class="row">
								<div class="form-group">
									...
									<div class="col-sm-6">
										<select class="form-control input-sm select2" name="supplier" id="supplier">
											<?php
											foreach ($supplier as $cs) {
											?>
												<option value="<?php echo $cs->id_suplier ?>"><?php echo $cs->nm_suplier ?></option>
											<?php } ?>
										</select>
							...
							<div class="form-group">
								...
								<div class="col-sm-6">
									<select class="form-control input-sm select2" name="bank" id="bank">
										<option value="">Pilih Bank</option>
										<option value="1102-01-01">OCBC NISP - IDR (Mcy) 103-8100-4848-0</option>
										<option value="1102-01-02">OCBC NISP - USD (Mcy) 103-8100-4848-0</option>
										<option value="1102-01-03">OCBC NISP - JPY (Mcy) 103-8100-4848-0</option>
										<option value="1102-01-04">OCBC NISP IDR (GIRO) 103-8000-0040-0</option>
										<option value="1102-01-05">BANK OF CENTRAL ASIA IN RUPIAH</option>
										<option value="1102-01-06">BANK OF CENTRAL ASIA IN US DOLLAR</option>
							...
							<div class="form-group ">
								<label class="col-sm-4 control-label">Pembayaran Bank</label>
								<div class="col-sm-6">
				...
				<div class="row">
					<div class="col-lg-12">
						...
						<button class="btn btn-danger">
							<a href="<?= base_url() ?>pembayaran/index_uang_muka">
								<i class="fa fa-refresh"></i><b> Kembali</b>
							</a>
						</button>

						<button id="simpanpenerimaan" class="btn btn-primary" type="button" onclick="savemutasi()">
							<i class="fa fa-save"></i><b> Simpan Data Pembayaran</b>
						</button>
					</div>
				</div>
			</div>
				...
				<div class="modal-body" id="MyModalBodyStok" style="background: #FFF !important;color:#000 !important;">
					<table class="table table-bordered" width="100%" id="list_item_stok">
						...
						<tbody>
							...
							$invoice = $this->db->query("SELECT * FROM tr_request WHERE id_suplier ='$cust'")->result();
							if ($invoice) {
	...
	<script>
				...
				swal({
					...
					function(isConfirm) {
						if (isConfirm) {
							...
							$.ajax({
								url: siteurl + "pembayaran/save_pembayaran_uangmuka",
								dataType: "json",
								type: 'POST',
								data: formdata,
		...
		*/
		...
		function startmutasi(id, surat, nm, avl, ppn, real) {
			var avl2 = numx(avl);
			var ppn2 = numx(ppn);
			var real2 = numx(real);
		...
		$(document).on('click', '.add', function() {
			...
			if (id_suplier == "") {
				...
				$.ajax({
					type: 'POST',
					url: siteurl + 'pembayaran/TambahRequest/' + id_suplier,
					data: {
