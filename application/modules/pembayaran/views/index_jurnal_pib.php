...
<div class="box">
		...
		<tr>
			...
			<td><?= number_format($record->jumlah_piutang_idr) ?></td>
			<td><?= number_format($record->biaya_ppn_idr) ?></td>
			<td><?= number_format($record->jumlah_bank_idr) ?></td>		
...
<script type="text/javascript">
	...
	$(document).on('click', '.view3', function(){
	    ...
	    var pp = 'JV007';
		var akses = 'jurnalpembayaranpib';
		// alert(id);
		$("#head_title").html("<i class='fa fa-list-alt'></i><b>Detail Jurnal</b>");
		$.ajax({
			type:'POST',
			url:siteurl+'jurnal_nomor/jurnal_fn/jurnal_pembayaran_pib/'+id+'/'+pp+'/'+akses,
			data:{'id':id},
			success:function(data){
				$("#dialog-popup").modal();
				$("#ModalView").html(data);
