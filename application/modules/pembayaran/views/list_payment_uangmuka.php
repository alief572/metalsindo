...
<form action="#" method="POST" id="form_proses_bro" enctype="multipart/form-data"> 
	<div class="box box-primary">
			...
			<table class="table table-bordered table-striped" id="example1" width='100%'> 
				<thead>
					<tr class='bg-blue'>
						...
						<th width='7%'>No Request</th>
						<th width='7%'>No PO</th>
						<th>Nama Supplier</th>
						<th class="text-right" width='7%'>Request</th>
		...
		<tr>
		    <td><?= $numb; ?></td>
			<td><?=  date('d-F-Y', strtotime($record->created_on)) ?></td>
			<td><?= $record->no_request ?></td>			
			<td><?= $record->no_surat ?></td>
			<td><?= $record->name_suplier ?></td>
			<td align="right"><?= number_format($record->nilai_invoice) ?></td>
			<td align="right"><?= number_format($record->nilai_ppn) ?></td>
			<td align="right"><?= number_format($record->grand_total) ?></td>			
         	<td style="padding-left:20px">
			<?php if($ENABLE_MANAGE) : ?>
				<button class='btn btn-sm btn-success terima' title='Bayar Uang Muka' data-request='<?=$record->no_request?>'><i class='fa fa-calculator'></i></button>
...
<script>
	...
	$(document).on('click', '.terima', function(e){
		e.preventDefault();	
		window.location.href = base_url +'pembayaran/modal_detail_request/'+$(this).data('request');
	});
