...
$id_coa 		= (!empty($data[0]->id_coa)) ? $data[0]->id_coa : '';
...
<div class="box box-primary">
	...
	<div class="box-body">
		...
		<div class='form-group row'>
			...
			<div class='col-sm-4'>
				<select name='branch' id='branch' class='form-control input-md chosen-select-asset-milik'>
			...
			<div class='col-sm-4'>
				<select name='category_pajak' id='category_pajak' class='form-control input-md chosen-select-category-pajak' <?= $dist; ?>>
		...
		<div class='form-group row'>
			...
			<div class='col-sm-4'>
				<select name='category' id='category' class='form-control input-md chosen-select-category' <?= $dist; ?>>
		...
		<div class='form-group row'>
			...
			<div class='col-sm-4'>
				<select name='lokasi_asset' id='lokasi_asset' class='form-control input-md chosen-select-department' <?= $dist; ?>>
			...
			<div class='col-sm-4'>
				...
				<?php
				if (empty($data[0]->id)) { ?>
					<select name='cost_center' id='cost_center' class='form-control input-md chosen-select-cost-center' <?= $dist; ?>>
		...
		<?php if (!empty($data[0]->id)) { ?>
			<div class='form-group row'>
				...
				<div class='col-sm-4'>
					<select name='lokasi_asset_new' id='lokasi_asset_new' class='form-control input-md chosen-select-lokasi-asset-new'>
				...
				<div class='col-sm-4'>
					<select name='cost_center_new' id='cost_center_new' class='form-control input-md chosen-select-cost-center-new'>
		...
		<div class='form-group row'>
			...
			<div class='col-sm-4'>
				<select name='penyusutan' id='penyusutan' class='form-control input-md' <?= $dist; ?>>
		...
		<div class='form-group row'>
			<label class='label-control col-sm-2'><b>Kelompok Penyusutan <span class='text-red'>*</span></b></label>
			<div class='col-sm-4'>
				<select name='kelompok_penyusutan' id='kelompok_penyusutan' class='form-control input-md chosen-select-kelompok-penyusutan' <?= $dist; ?>>
					<option value="">- Pilih Kelompok Penyusutan -</option>
					<?php
					foreach ($list_coa_asset as $item) {
						$selected = '';
						if ($item['no_perkiraan'] == $id_coa) {
							$selected = 'selected';
						}
...
<?php } ?>
<script src="hhttps://cdnjs.cloudflare.com/ajax/libs/chosen/1.8.7/chosen.jquery.min.js"></script>
<script>
	$(function() {
	...
	$(document).ready(function() {
		$('.chosen-select-asset-milik').chosen({
			width: '100%'
		});
		$('.chosen-select-category-pajak').chosen({
			width: '100%'
		});
		...
		$('.chosen-select-cost-center').chosen({
			width: '100%'
		});
		$('.chosen-select-lokasi-asset-new').chosen({
			width: '100%'
		});
		$('.chosen-select-cost-center-new').chosen({
			width: '100%'
		});
		$('.chosen-select-kelompok-penyusutan').chosen({
			width: '100%'
		});
