...
$qData	= "SELECT a.*, b.nm_dept FROM asset a LEFT JOIN department_center b ON a.cost_center=b.id WHERE a.id='" . $this->uri->segment(3) . "'";
...
$list_coa = $this->Asset_model->getList(DBACC . '.coa_master');
...
<div class="box box-primary">
	...
	<div class="box-body">
		...
		<div class='form-group row'>
			...
			<div class='col-sm-4'>
				<?php
				echo form_input(array('id' => 'cost_center', 'name' => 'cost_center', 'class' => 'form-control input-md', 'autocomplete' => 'off', 'placeholder' => 'Nilai Asset', 'readonly' => 'readonly'), strtoupper($dataD[0]['nm_dept']));
				?>
			</div>
		</div>
		<div class='form-group row'>
		...
		<?php if ($dataD[0]['penyusutan'] == 'Y') { ?>
			<div class='form-group row'>
				...
				<div class='col-sm-4'>
					<?php
					echo form_input(array('id' => 'depresiasi', 'depresiasi' => 'value', 'class' => 'form-control input-md', 'autocomplete' => 'off', 'placeholder' => 'Dipresiasi Perbulan', 'readonly' => 'readonly'), $dataD[0]['depresiasi'] . " Tahun");
					?>
				</div>
		...
		<div class='form-group row'>
			...
			<div class='col-sm-4'>
				<select name='kelompok_penyusutan' id='kelompok_penyusutan' class='form-control input-md chosen-select' disabled>
					<option value="">- Pilih Kelompok Penyusutan -</option>
					<?php
					foreach ($list_coa as $item) {
						$selected = '';
						if ($item['no_perkiraan'] == $dataD[0]['id_coa']) {
							$selected = 'selected';
						}
