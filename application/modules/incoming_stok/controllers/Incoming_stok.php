...
class Incoming_stok extends Admin_Controller
{
  ...
  public function request_stok($id = null)
  {
    if ($this->input->post()) {
      ...
      if (!empty($data['Detail'])) {
        foreach ($detail as $val => $valx) {
          ...
          if ($qty_incoming > 0) {
            ...
            $getIncoming  = $this->db->get_where('dt_trans_po_non_material', array('id' => $valx['id']))->result_array();
      ...
      if (!empty($ArrUpdatePO)) {
        $this->db->update_batch('dt_trans_po_non_material', $ArrUpdatePO, 'id');
      }
      ...
      $countListNomorPO = $this->db
        ...
        ->join('tr_purchase_order_non_material b', 'a.no_po=b.no_po', 'left')
      ...
      $get_list_supplier = $this->db->select('id_suplier as kode_supplier, name_suplier as nama')->get_where('master_supplier', ['deleted_by' => null])->result();
  ...
  public function print_incoming_stok()
  {
    ...
    $no_po = [];
    $get_no_po = $this->db->query("SELECT a.no_surat FROM tr_purchase_order_non_material a WHERE a.no_po IN ('" . str_replace(",", "','", $getData[0]['no_ipp']) . "')")->result();
    ...
    if (!empty($listNomorPO)) {
      foreach ($listNomorPO as $item) {
        ...
        $get_no_pr = $this->db->query("
          ...
          WHERE
            a.id IN (SELECT aa.idpr FROM dt_trans_po_non_material aa WHERE aa.no_po = '" . $item->no_po . "' AND (aa.tipe IS NULL OR aa.tipe = ''))
          GROUP BY b.no_pr
          ...
          WHERE
            a.id IN (SELECT aa.idpr FROM dt_trans_po_non_material aa WHERE aa.no_po = '" . $item->no_po . "' AND aa.tipe = 'pr depart')
          GROUP BY b.no_pr
        ")->result();
