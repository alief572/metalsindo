...
class Incoming_stok_model extends BF_Model
{
  ...
  public function data_side_request_material()
  {
    ...
    foreach ($query->result_array() as $row) {
      ...
      $no_po = [];
      $get_no_po =  $this->db->query("SELECT a.no_surat FROM tr_purchase_order_non_material a WHERE a.no_po IN ('" . str_replace(",", "','", $row['no_ipp']) . "')")->result();
      ...
      $get_no_pr = $this->db->query("
          ...
          FROM
            dt_trans_po a
            JOIN tr_purchase_order_non_material d ON d.no_po = a.no_po
  ...
  public function get_query_json_request_material($like_value = NULL, $column_order = NULL, $column_dir = NULL, $limit_start = NULL, $limit_length = NULL)
  {
    ...
    $sql = "SELECT
            ...
            FROM
              ...
              LEFT JOIN tr_purchase_order_non_material e ON a.no_ipp=e.no_po,
              (SELECT @row:=0) r
