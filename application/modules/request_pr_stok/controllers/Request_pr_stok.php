...
class Request_pr_stok extends Admin_Controller
{
  ...
  public function index()
  {
    ...
    $get_data = $this->db->select('a.*, b.name_customer, e.nm_lengkap as request_by, DATE_FORMAT(a.created_date, "%d %M %Y") as request_date')
  ...
  public function PrintH2()
  {
    ...
    $data['header'] = $this->db->query("SELECT a.*, b.name_customer as nm_customer, b.address_office as alamat, b.fax FROM material_planning_base_on_produksi as a LEFT JOIN material_planning_base_on_produksi x ON x.so_number = a.so_number LEFT JOIN master_customers b ON b.id_customer = a.id_customer WHERE a.so_number = '" . $id . "' ")->result();
    $data['detail']  = $this->db->query("SELECT a.*, e.id_stock as code, e.stock_name as nama, if(e.konversi <= 0, 1, e.konversi) as konversi, f.code as satuan, g.code as satuan_packing FROM material_planning_base_on_produksi_detail a 
