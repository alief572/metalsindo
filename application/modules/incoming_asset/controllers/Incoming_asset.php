...
class Incoming_asset extends Admin_Controller
{
	...
	{
		$this->db->select('"PO" AS ket, a.no_surat as no_po, b.name_suplier as nm_supplier');
		...
		$this->db->where('(SELECT IF(SUM(aa.qty_in) IS NULL, 0, SUM(aa.qty_in)) FROM dt_trans_po_non_material aa WHERE aa.no_po = a.no_po) <=', 0);
	...
	public function server_side_incoming_asset()
	{
		...
		$urut2  = 0;
		// $GET_USERNAME = get_detail_user();
		$GET_USERNAME = $this->db->get_where('users', ['id_user' => $this->auth->user_id()])->row();
		foreach ($query->result_array() as $row) {
			$total_data     = $totalData;
			$start_dari     = $requestData['start'];
			$asc_desc       = $requestData['order'][0]['dir'];
			...
			$TANGGAL = (!empty($row['tanggal'])) ? $row['tanggal'] : $row['created_date'];
			$NM_LENGKAP = (!empty($GET_USERNAME->nm_lengkap)) ? $GET_USERNAME->nm_lengkap : $row['created_by'];
			$print	= "&nbsp;<a href='" . base_url('incoming_asset/print_incoming_assets/' . $row['kode_trans']) . "' target='_blank' class='btn btn-sm btn-warning' title='Print'><i class='fa fa-print'></i></a>";
	...
	public function modal_incoming_asset()
	{
		...
		// $result = $this->db->get_where('dt_trans_po_non_material', ['no_po' => $get_po['no_po']])->result_array();
	...
	public function process_incoming_asset()
	{
		...
		if ($adjustment == 'IN') {
			...
			foreach ($addInMat as $val => $valx) {
				...
				$GET_UNITPRICE = $this->db->select('hargasatuan as unit_price, id as id_barang')->get_where('dt_trans_po_non_material', ['id' => $valx['id']])->result();
			...
			$update_po_detail = $this->db->update_batch('dt_trans_po_non_material', $ArrUpdate, 'id');
			if (!$update_po_detail) {
