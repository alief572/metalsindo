...
<div class="box">
	<div class="box-body">
		...
		<?php if(empty($results)){
			...
			$tgl	= $this->db->query("SELECT a.* FROM tr_spk_aktual a WHERE a.id_spkproduksi='$id_spkproduksi' ")->row();
			
			// print_r($id_spkproduksi);
			// exit();
			
			
			$produksi_date = (!empty($tgl->date_production))?date('d-m-Y', strtotime($tgl->date_production)):date('d-m-Y', strtotime($record->tgl_spk_produksi));
			?>
		<tr>
		    <td><?= $numb; ?></td>
			...
			<?php if($ENABLE_VIEW) : ?>
				<a class="btn btn-success btn-sm view" href="<?= base_url('/spk_aktual/addHeader_view/'.$record->id_tr_spk_produksi.'/view') ?>" title="View"><i class="fa fa-eye"></i>
				</a>
			<?php endif; ?>
			</td>
			<?php }else{ ?>
			<td style="padding-left:20px">
			<?php if($ENABLE_VIEW) : ?>
				<a class="btn btn-warning btn-sm view" href="<?= base_url('/spk_aktual/addHeader_view/'.$record->id_tr_spk_produksi.'/view') ?>" title="View"><i class="fa fa-eye"></i>
				</a>
			<?php endif; ?>
...
<div class="box">
	<div class="box-body">
		...
		<tr>
			...
			<td style="padding-left:20px">
			<?php if($ENABLE_VIEW) : ?>
				<a class="btn btn-warning btn-sm view" href="<?= base_url('/spk_aktual/addHeader_view/'.$aktual->id_spk_aktual.'/view') ?>" title="View"><i class="fa fa-eye"></i>
				</a>
				...
				<a class="btn btn-success btn-sm" href="<?= base_url('/spk_aktual/TambahLHP/'.$aktual->id_spk_aktual.'/view') ?>"  title="View LHP Waktu"><i class="fa fa-eye"></i></i></a>
