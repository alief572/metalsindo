...
class Spk_marketing extends Admin_Controller
{
	...
	public function index_slitting()
	{
		$this->auth->restrict($this->viewPermission);
		$session = $this->session->userdata('app_session');
		$this->template->page_icon('fa fa-users');
		$data = $this->Inventory_4_model->CariSPKSlitting();
		$this->template->set('results', $data);
		$this->template->title('SPK Marketing Slitting');
	...
	public function revisiHeader()
	{
		...
		$this->template->render('RevisiHeader');
	}
	...
	public function ViewHeaderslitting($id)
	{
		$id = $this->uri->segment(3);
		$this->auth->restrict($this->viewPermission);
		$session = $this->session->userdata('app_session');
		...
		$this->template->set('results', $data);
		$this->template->title('View Penawaran');
		$this->template->render('ViewHeaderslitting');
	}
	...
	function GetTipe()
	{
		...
		$dt1	= $this->db->query("SELECT * FROM tr_penawaran WHERE no_penawaran = '$no_penawaran' ")->result();
		$tipe = $dt1[0]->type;
		echo "<div class='col-md-4'><label for='tipe'>Tipe</label></div>
			<div class='col-md-8'><input type='text' value='$tipe' class='form-control' id='tipe' required name='tipe' readonly ></div>";
	}
	...
	function GetPenawaranSlitting()
	{
		...
		foreach ($dt as $dt) {
			$loop++;
			$id_category3 = $dt->id_category3;
			$harga = $dt->harga_penawaran_cust;
			$qty = $dt->qty_slitting;
			$tharga = $harga * $qty;
			$harga_fmt = number_format($harga, 2, ',', '.');
			$qty_fmt = number_format($qty, 2, ',', '.');
			$tharga_fmt = number_format($tharga, 2, ',', '.');
			$crcl	= $this->db->query("SELECT * FROM dt_inquery_transaksi WHERE no_inquery = '$idcr' AND id_category3='$id_category3' ")->result();
			echo "
		<tr id='tabel_penawaran_$loop'>
			<th hidden><input type='text' class='form-control' value='$dt->id_child_penawaran' readonly id='dp_id_child_penawaran_$loop' required name='dp[$loop][id_child_penawaran]'></th>
			...
			<th><input type='text' class='form-control' value='$dt->nama3|$dt->maker' readonly id='dp_noalloy_$loop' required name='dp[$loop][noalloy]'></th>
			<th><input type='text' class='form-control' value='$dt->thickness' readonly id='dp_thickness_$loop' required name='dp[$loop][thickness]'></th>
			<th><input type='text' class='form-control' value='$dt->width' id='dp_width_$loop' required name='dp[$loop][width]' readonly></th>
			...
			<th><input type='text' class='form-control nominal-format' value='$harga_fmt' readonly id='dp_hgpenwaran_$loop' required name='dp[$loop][hgpenaaran]'></th>
			<th><input type='text' class='form-control nominal-format' value='$harga_fmt' onchange='return AksiDetail($loop);' id='dp_hgdeal_$loop' required name='dp[$loop][hgdeal]'></th>
			<th ><input type='text' class='form-control nominal-format' onchange='return AksiDetail($loop);' id='dp_qty_$loop' required name='dp[$loop][qty]' value='$qty_fmt'></th>
			<th hidden><input type='text' class='form-control' onchange='return AksiDetail($loop);'id='dp_weight_$loop' required name='dp[$loop][weight]'></th>
			<th id='total_weight_$loop' hidden><input type='text' class='form-control' value='$jcc'  id='dp_twight_$loop' required name='dp[$loop][twight]'></th>
			...
			<input type='text' class='form-control' value='$tharga_fmt' readonly></th>
			<th><input type='date' class='form-control'   id='dp_ddate_$loop' data-role='qtip' required name='dp[$loop][ddate]'></th>
			...
			echo "</select></th>
		    <th><input type='text' class='form-control' value='$dt->estimasi' id='dp_estimasi_$loop' required name='dp[$loop][estimasi]'></th>
			<th><input type='text' class='form-control' value='$dt->qty_coil' id='dp_qty_coil_$loop' required name='dp[$loop][qty_coil]'></th>
			<th id='total_keterangan_$loop' hidden><textarea class='form-control' id='dp_keterangan_$loop' required name='dp[$id][keterangan]' rows='2'></textarea> </th>
			<th><input type='checkbox' value='1' id='dp_deal_$loop' required name='dp[$loop][deal]'></th>
		</tr>
		";
		};
	...
	public function Approve()
	{
		...
		$this->db->where('id_spkmarketing', $id)->update("dt_spkmarketing", $data2);
	...
	public function SaveNewHeaderSlitting()
	{
		...
		foreach ($_POST['dp'] as $dp) {
			...
			$stokpakai =  array(
				...
				'type'					=> $post['tipe'],
				'qty_slitting'			=> $dp[qty],
				'qty_coil'				=> $dp[qty_coil],
				'width_akhir'			=> $dp[width_akhir],
				'estimasi'				=> $dp[estimasi]
	...
	public function SaveRevisiHeader()
	{
		$this->auth->restrict($this->addPermission);
		$post = $this->input->post();
	...
	public function get_penawaran_slitting()
	{
		$id 	= $this->input->post('id');
		$result	= $this->db->get_where('tr_penawaran', array('id_customer' => $id, 'status' => 'N', 'type' => 'slitting'))->result_array();
	...
	public function get_penawaran_revisi()
	{
		$id 			= $this->input->post('id');
		$no_penawaran 	= $this->input->post('no_penawaran');
		$result	= $this->db->get_where('tr_penawaran', array('no_penawaran' => $no_penawaran))->result_array();
	...
	public function PrintH2()
	{
		...
		$data['detail']  	= $this->db
			...
			->join('ms_surface e', 'e.id_surface=c.id_surface')
	...
	{
		$hapus = $this->db->query("SELECT * FROM hapus_dt")->result();

		foreach ($hapus as $val) {
			$id = $val->id;
			//$this->db->query("DELETED FROM dt_spkmarketing_dummy WHERE id='$id'");
			$this->db->delete('dt_spkmarketing_dummy', array('id' => $id));
		}
	}
