...
<div class="box box-primary">
	<div class="box-body">
		<form id="data-form" method="post">
									...
									<table class='table table-bordered table-striped' style='min-width: 1400px; white-space: nowrap;'>
										<thead>
											<tr class='bg-blue'>
												...
												<th style="min-width:80px;">Qty <?= (isset($results['tipe_sheet']) && $results['tipe_sheet'] == 1) ? 'Sheet' : 'Kg' ?></th>
			...
			<th><input type='text' class='form-control' 	value='" . (isset($dt->part_number) ? $dt->part_number : '') . "' id='dp_part_number_$loop' name='dp[$loop][part_number]'></th>
...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		$.ajax({
			url: siteurl + 'spk_marketing/get_penawaran_revisi',
			cache: false,
			type: "POST",
			data: {
				'id': id_customerx,
			...
			swal({
				...
				function(isConfirm) {
					if (isConfirm) {
						...
						var baseurl = siteurl + 'spk_marketing/SaveRevisiHeader';
						$.ajax({
							url: baseurl,
							type: "POST",
							data: formData,
