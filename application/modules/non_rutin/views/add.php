			...
			<div class='form-group row'>
				...
				<div class='col-sm-4  text-right'>
					...
					<?php if (!empty($upload_spk)) { ?>
						<a href='<?= base_url('uploads/PR/' . $upload_spk); ?>' target='_blank' title='Download' data-role='qtip'>Download</a>
					<?php } ?>
				</div>
...
<script>
	...
	function sum_total(a) {
		var qty = $('#qty_' + a).val().split(",").join("");
		var harga = $('#harga_' + a).val().split(",").join("");
