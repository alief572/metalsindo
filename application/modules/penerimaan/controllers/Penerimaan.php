...
class Penerimaan extends Admin_Controller
{
	...
	public function get_invoice_serverside()
	{
		...
		$count_all = $db_total->count_all_results();

		// Search filter
		if (!empty($search['value'])) {
			$this->db->group_start();
			$this->db->like('a.no_invoice', $search['value'], 'both');
		...
		foreach ($get_data as $item) {
			...
			// Action button
			if ($item->printed_on == null) {
				$action = '<span class="text-muted">Belum cetak</span>';
			} else {
				$action = '<button id="btn-' . $item->no_invoice . '" class="btn btn-warning btn-sm" type="button" onclick="startmutasi(\'' . $item->no_invoice . '\', \'' . $item->no_surat . '\',\'' . addslashes($item->nm_customer) . '\',\'' . $nilai_invoice . '\',\'' . $sisa_invoice_idr . '\')">Pilih</button>';
			}

			$hasil[] = [
				'code'          => $item->no_invoice,
				'no_invoice'    => $item->no_surat,
				'nama_customer' => $item->nm_customer,
				'total_invoice' => number_format($nilai_invoice),
				'sisa_invoice'  => number_format($sisa_invoice_idr),
				'action'        => $action
			];
		}
