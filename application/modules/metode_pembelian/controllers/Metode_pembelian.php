...
class Metode_pembelian extends Admin_Controller
{
	...
	public function modal_detail_pr()
	{
		...
		if ($tipe_pr == 'Department') {
			...
			$this->db->select('c.stock_name AS nama_material, a.propose_purchase as qty, d.tgl_dibutuhkan as tgl_dibutuhkan');
			...
			if ($total != 0) {
				foreach ($datajurnal1 as $rec) {
					...
					if ($rec->parameter_no == "4") {
						$det_Jurnaltes1[] = array(
							'nomor' => $nomor_jurnal, 'tanggal' => $payment_date, 'tipe' => 'JV', 'no_perkiraan' => $rec->no_perkiraan, 'keterangan' => 'Potongan DP ' . $datapo->no_po, 'no_reff' => $data['invoice_no'], 'debet' => 0, 'kredit' => $data['potong_um'], 'no_request' => $datapo->no_po, 'jenis_jurnal' => $jenis_jurnal, 'nocust' => $datapo->id_supplier, 'stspos' => '1'
						);
					}
				}
			}
