										...
										<select id="id_material" name="id_material" class="form-control select" onchange="Caristock()" required>
											<option value="">--Pilih--</option>
												<?php foreach ($results['material'] as $material){
													...
													?>
											<option value="<?= $material->id_stock?>" <?=$selx;?>><?= ucfirst(strtolower($material->nama))?>|<?= ucfirst(strtolower($material->maker))?>|<?= ucfirst(strtolower($material->name_customer))?>|<?= 'Lebar :'?><?= $material->width?>|<?= 'Berat :'?><?= $material->weight?></option>
												<?php } ?>
										</select>
								</div>
							</div>
...
<script type="text/javascript">
			...
			swal({
				...
				function(isConfirm) {
				  if (isConfirm) {
						...
						var baseurl=siteurl+'spk_produksi/SaveNewHeaderBooking';
						$.ajax({
							url			: baseurl,
							type		: "POST",
							data		: formData,
	...
	const changeChecked = () => {
		...
		let efisiensiberat   = SUM_KG/dataX.kg_process;
