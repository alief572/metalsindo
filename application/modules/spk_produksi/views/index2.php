						...
						<table id="example2" class="table table-bordered table-striped">
							...
							<tbody>
								...
								<tr>
									...
									<?php endif; ?>
									<?php if($ENABLE_VIEW) : ?>
										<a class="btn btn-success btn-sm approve" href="javascript:void(0)" title="Turunkan" data-id_spkproduksi="<?=$record->id_tr_spk_produksi?>"><i class="fa fa-check"></i>
										</a>
									<?php endif; ?>
									</td>
									<?php }else{ ?>
...
<script type="text/javascript">
	...
	$(document).on('click', '.approve', function(e){
		...
		function(){
		  $.ajax({
			  type:'POST',
			  url:siteurl+'spk_produksi/Approve_Booking',
			  dataType : "json",
			  data:{'id':id},
			  success:function(result){
				  if(result.status == '1'){
