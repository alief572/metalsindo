<?php
	...
	$kg_process = (!empty($header1))?$header1[0]->kg_process:'';
	$process1 = (!empty($header1))?$header1[0]->process:'';
	$length = (!empty($header))?$header[0]->length:'';
	$count_m = (!empty($header))?$header[0]->count_m:'';

	$id_stock = (!empty($header1))?$header1[0]->id_stock:'';
					...
					<div class="col-sm-12">
						...
						<div class="col-sm-6">
							<div class="form-group row">
								...
								<div class="col-md-4">
									<input type='text' class='form-control' id='kg_process' name='kg_process' readonly   value='<?=$process;?>'>
								</div>
							</div>
						</div>
...
<script type="text/javascript">
	...
	const calDetail = (a) => {
		...
		let hasil_kg 	        = (dataX.used_weight * dataX.kg_process / dataX.width).toFixed(2);
		let total 		        = (dataX.used_qtycoil * hasil_kg).toFixed(2);
		let totalpanjang 		= (dataX.used_qtycoil * dataX.used_panjang).toFixed(2);
		
		
		
		$('#used_weight2_'+a).val(hasil_kg);
		$('#used_totalwidth_'+a).val(total);
		$('#used_totalpanjang_'+a).val(totalpanjang);
