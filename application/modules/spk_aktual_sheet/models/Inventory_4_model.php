...
class Inventory_4_model extends BF_Model
{
	...
	public function get_data_material(){
		$search = "a.deleted='0'";
		$bentuk = "a.id_bentuk='B2000002'";
		$this->db->select('a.*');
		$this->db->from('ms_inventory_category3 a');
		$this->db->where($search);
		$this->db->where($bentuk);
		$query = $this->db->get();		
		return $query->result();
	...
	public function CariheaderSPK($id){
		$this->db->select('a.*');
		$this->db->from('tr_spk_produksi_sheet a');
	...
	public function CaridetailSPK($id){
		$this->db->select('a.*, c.no_surat as no_spkmarketing, d.name_customer');
		$this->db->from('dt_spk_produksi_sheet a');
		$this->db->join('dt_spkmarketing b','b.id_dt_spkmarketing=a.no_surat');
		$this->db->join('tr_spk_marketing c','c.id_spkmarketing=b.id_spkmarketing');
		$this->db->join('master_customers d','d.id_customer=c.id_customer');
		$this->db->where('a.id_spkproduksi',$id);
	...
	public function CarimaterialSPK($id){
		$this->db->select('a.*');
		$this->db->from('dt_material_sheet a');	
		$this->db->where('a.id_dt_spkmarketing',$id);
		$query = $this->db->get();	
		return $query->result();
	}
