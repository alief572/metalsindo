...
<div class="box">
	<div class="box-body">
		...
		<?php if(empty($results)){
			...
			$customer	= $this->db->query("SELECT a.*, b.name_customer as name_customer FROM dt_spk_produksi_sheet as a INNER JOIN master_customers as b ON a.idcustomer = b.id_customer WHERE a.id_spkproduksi='$id_spkproduksi' GROUP BY a.idcustomer ")->result();
			
			$tgl	= $this->db->query("SELECT a.* FROM tr_spk_aktual a WHERE a.id_spk_aktual='$id_spkproduksi' ")->row();
			
			$produksi_date = (!empty($tgl->date_production))?date('d-m-Y', strtotime($tgl->date_production)):'-';
			?>
		<tr>
		    <td><?= $numb; ?></td>
			...
			<?php if($ENABLE_VIEW) : ?>
				<a class="btn btn-success btn-sm view" href="<?= base_url('/spk_aktual_sheet/addHeader/'.$record->id_tr_spk_produksi.'/view') ?>" title="View"><i class="fa fa-eye"></i>
				</a>
			<?php endif; ?>
			...
			<td style="padding-left:20px">
			<?php if($ENABLE_VIEW) : ?>
				<a class="btn btn-warning btn-sm view" href="<?= base_url('/spk_aktual_sheet/addHeaderproses/'.$record->id_spkproduksi.'/view') ?>" title="View"><i class="fa fa-eye"></i>
				</a>
			<?php endif; ?>

			<!-- <a class="btn btn-info btn-sm" href="<?= base_url('/spk_aktual_sheet/printSPKProduksi/'.$record->id_spkproduksi) ?>" target="_blank"  title="Print"><i class="fa fa-print"></i>
				</a> -->

			<?php if($ENABLE_MANAGE) : ?>
			<a class="btn btn-success btn-sm" href="<?= base_url('/spk_aktual_sheet/EditHeader/'.$record->id_spkproduksi) ?>"  title="Input LHP Material Gabungan"><i class="fa fa-plus"></i></i></a>
			
			<a class="btn btn-danger btn-sm" href="<?= base_url('/spk_aktual_sheet/EditHeadernew/View/'.$record->id_tr_spk_produksi) ?>"  title="View LHP Material"><i class="fa fa-eye"></i></i></a>
				</a>
			<a class="btn btn-success btn-sm" href="<?= base_url('/spk_aktual_sheet/EditHeadernew/Edit/'.$record->id_tr_spk_produksi) ?>"  title="Edit LHP Material"><i class="fa fa-pencil"></i></i></a>
				</a>
			
			<?php endif; ?>
			<?php if($ENABLE_MANAGE) : ?>
			<a class="btn btn-success btn-sm" href="<?= base_url('/spk_aktual_sheet/TambahLHP/'.$record->id_spkproduksi) ?>"  title="Input LHP"><i class="fa fa-edit"></i></i></a>
				</a>
			<?php endif; ?>
...
<div class="box">
	<div class="box-body">
		...
		<?php if(empty($results)){
			...
			$customer	= $this->db->query("SELECT a.*, b.name_customer as name_customer FROM dt_spk_produksi_sheet as a INNER JOIN master_customers as b ON a.idcustomer = b.id_customer WHERE a.id_tr_spk_produksi='$id_spkproduksi' GROUP BY a.idcustomer ")->result();
		...
		<tr>
			...
			<td style="padding-left:20px">
			<?php if($ENABLE_VIEW) : ?>
				<a class="btn btn-warning btn-sm view" href="<?= base_url('/spk_aktual_sheet/addHeader/'.$aktual->id_spk_aktual.'/view') ?>" title="View"><i class="fa fa-eye"></i>
				</a>
				
				<a class="btn btn-success btn-sm" href="<?= base_url('/spk_aktual_sheet/EditHeadernew/Edit/'.$aktual->id_spk_aktual.'/history') ?>"  title="Edit LHP Material"><i class="fa fa-pencil"></i></i></a>
				
				<a class="btn btn-danger btn-sm" href="<?= base_url('/spk_aktual_sheet/EditHeadernew/View/'.$aktual->id_spk_aktual) ?>"  title="View LHP Material"><i class="fa fa-eye"></i></i></a>
				</a>				
...
<script type="text/javascript">
	...
	$(document).on('click', '.delete', function(e){
		...
		function(){
		  $.ajax({
			  type:'POST',
			  url:siteurl+'spk_aktual_sheet/Approve_new',
			  dataType : "json",
			  data:{'id':id},
