...
?>
 ...
 <div class="box box-primary">
    <div class="box-body">
		...
		<tr>
		<th hidden><input type='text' class='form-control'  value='<?= $tr_spk->id_spkproduksi?>' 	readonly id='nomor' required name='nomor'></th>
		...
		<div class="col-sm-12">
			...
			<table class='table table-bordered table-striped' >
			<thead>
			<tr class='bg-yellow' valign='top'>
            <th width='3%'>No</th>
            <th width='3%'>SPK Marketing</th>
            <th width='5%'>Customer</th>
			<th width='3%'>Thickness</th>
			<th width='3%'>Density</th>
            <th width='3%'>Width FG</th>
			<th width='3%'>Length</th>
            <th width='3%'>Weight/Sheet</th>
            <th width='15%'> Lot Number Sheet</th>
			<th width='5%'>Qty Sheet</th>
			<th width='5%'>Actual <br>Weight<br>Sheet</th>
			...
			foreach ($results['dt_spk'] as $dt_spk){ 
				...
				$spk = $dt_spk->id_spkproduksi;
				$dtspkproduksi = $dt_spk->id_dt_spkproduksi;
				
			                  $spkproduksi = $this->db->query("SELECT * FROM tr_spk_produksi_sheet WHERE id_spkproduksi='$spk'")->row();
							  $idmaterial  = $spkproduksi->id_material;
							  
							  $mat = $this->db->query("SELECT * FROM ms_inventory_category3 WHERE id_category3='$idmaterial'")->row();
							  
							  $length = $this->db->query("SELECT a.*, b.length_sheet FROM dt_spk_produksi_sheet a 
							  INNER JOIN dt_spkmarketing b on a.no_surat=b.id_dt_spkmarketing 
 
							  WHERE a.id_dt_spkproduksi='$dtspkproduksi'")->row();
			      
			                  $sum_width += $dt_spk->weight;
							  $countfiles = 1;
							  
							  $thicknessfg = $spkproduksi->thickness;
							  $widthfg     = $dt_spk->weight_order;
							  $lengthfg    = $length->length_sheet;
 							  $densityfg   = $spkproduksi->density;
							  
							  $beratperkg =  $thicknessfg* $widthfg*$lengthfg*$densityfg;
							  $perkg = round($beratperkg/1000000,2);
							  ...
							  for($loop1=0;$loop1<$countfiles;$loop1++){
								...
								<tr id='tr_$i$loop'>
									<th>$loop</th>
									<th ><input type='text' class='form-control' 	value='$spkproduksi->no_surat' 	readonly id='used_nosurat_$i$loop' required name='dt[$i$loop][nosurat]'></th>
									...
									<th><input type='text' class='form-control'  	   	value='$dt_spk->namacustomer' 	readonly id='used_namacustomer_$i$loop' required name='dt[$i$loop][namacustomer]'></th>
									<th hidden><input type='text' class='form-control' 		value='$spkproduksi->nama_material'  	readonly id='used_nmmaterial_$i$loop' required name='dt[$i$loop][nmmaterial]'></th>
									
									<th> <input type='text' class='form-control' 		value='$spkproduksi->thickness' 		readonly id='used_thickness_$i$loop' required name='dt[$i$loop][thickness]'></th>
									
									<th> <input type='text' class='form-control' 		value='$spkproduksi->density' 		readonly id='used_density_$i$loop' required name='dt[$i$loop][density]'></th>
									
									<th><input type='text' class='form-control'   		value='$dt_spk->weight_order' 	 id='used_weight_$i$loop' required name='dt[$i$loop][weight][]' readonly></th>
									
									<th hidden>
									<input type='text' class='form-control'   		value='$dt_spk->width' 			 id='used_width_$i$loop' required name='dt[$i$loop][width][]'>
									<input type='hidden' class='form-control'   		value='$dt_spk->totalwidth' 		readonly id='used_totalwidth_$i$loop' required name='dt[$i$loop][totalwidth][]'></th>
									 ...
									 <th><input type='text' class='form-control'   		value='$length->length_sheet' 	 id='used_length_$i$loop' required name='dt[$i$loop][length][]' readonly></th>  
								
									
									<th>
									  <input type='text' class='form-control'   		value='$perkg' 	 id='used_widthperkg_$i$loop' required name='dt[$i$loop][widthperkg][]' readonly>
									</th>
									
									<th>
									  <input type='text' class='form-control'   		value='$tr_spk->lotno-$i' 		readonly id='used_lot_slitting_$i$loop' required name='dt[$i$loop][lot_slitting][]'>
									</th>
									
									<th> 
									<input type='hidden' class='form-control' value='0' readonly id='used_qtycoil_$i$loop'  required name='dt[$i$loop][qtycoil]'>
									
									<input type='text' class='form-control' value='0'  id='used_qtysheet_$i$loop'  required name='dt[$i$loop][qtysheet]'>
									</th>
									...
									<input type='hidden' class='form-control'   		value='$dt_spk->id_dt_spkproduksi' id='used_id_dt_spkproduksi_$i$loop' required name='dt[$i$loop][id_dt_spkproduksi][]'>
									...
									<th>
									<input type='text' class='form-control datepicker'    		value='$dt_spk->delivery' 		 id='used_delivery_$i$loop' required name='dt[$i$loop][delivery][]'>
	    ...
	    <tr>
            ...
            <td hidden width='15%'><input type='text' class='form-control autoNumeric' name='ngin_lebar[]' value='0' placeholder='Lebar' ></td>
		...
		<tr>
            ...
            <td hidden width='15%'><input type='text' class='form-control autoNumeric' name='ngek_lebar[]' value='0' placeholder='Lebar' ></td>
...
<script type="text/javascript">
			...
			swal({
				...
				function(isConfirm) {
				  if (isConfirm) {
						...
						var baseurl=siteurl+'spk_aktual_sheet/SaveEditHeader';
						$.ajax({
							url			: baseurl,
							type		: "POST",
							data		: formData,
	...
	function HitungTotalberat(id){
		...
		var perkg=$('#used_widthperkg_'+id).val();
		
		var totalBerat = getNum(qtycoil) * getNum(width);
		var totalsheet = Math.floor(getNum(width)/getNum(perkg));
		
		$('#used_totalaktual_'+id).val(totalBerat);
		$('#used_qtysheet_'+id).val(totalsheet);
