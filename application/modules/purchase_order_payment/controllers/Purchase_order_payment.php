...
class Purchase_order_payment extends Admin_Controller
{
	...
	public function view_inc()
	{
		...
		$this->template->set('data_invoice', $get_invoice);
		$this->template->set('data_detail_incoming', $get_detail_incoming);
	...
	public function save_invoice()
	{
		...
		if ($post['tipe_req'] == 'dp') {
			...
			if (count($check_po) < 1) {
				...
				$update_kurs       = $this->db->update('tr_purchase_order_non_material', ['kurs_terima_invoice_progress' => $kurs], ['no_surat' => $no_po]);
				if (!$update_kurs) {
					print_r($this->db->error($update_kurs));
					exit;
				}
		...
		if ($post['tipe_req'] == 'dp') {
			...
			$kode_supplier = $get_supplier->id_suplier;
			$nama = $get_supplier->name_suplier;
		} else {
		...
		$data_po     = $this->db->query("select * from tr_purchase_order_non_material WHERE no_surat = '$no_po'")->row();
	...
	public function search_inc()
	{
		...
		$get_supplier = $this->db->get_where('master_supplier', ['id_suplier' => $kode_supplier])->row();

		$this->db->select('a.*');
		$this->db->from('tr_invoice_po a');
		$this->db->like('a.no_po', 'TR', 'both');
		if ($kode_supplier !== '') {
			$this->db->where('a.id_supplier', $kode_supplier);
		}
		$get_list_inc = $this->db->get()->result_array();
		...
		foreach ($get_list_inc as $item) {
			...
			$hasil .= '<td style="text-align: center;">' . $item['no_po'] . '</td>';
			$hasil .= '<td style="text-align: center;">' . $no_po . '</td>';
			$hasil .= '<td style="text-align: center;">' . $item['invoice_no'] . '</td>';
			$hasil .= '<td style="text-align: center;">' . $no_payment . '</td>';
			$hasil .= '<td style="text-align: center;">' . date('d F Y', strtotime($item['invoice_date'])) . '</td>';
			$hasil .= '<td>' . $item['nm_supplier'] . '</td>';
			$hasil .= '<td style="text-align: right">' . number_format($item['total_invoice'], 2) . '</td>';
	...
	public function search_dp()
	{
		...
		$this->db->select('a.*, b.nm_lengkap, c.name_suplier as nm_supplier, IF(SUM(d.persen_dp) IS NULL, 0, SUM(d.persen_dp)) as ttl_persen_dp, e.id as id_top, e.progress, e.nilai as nilai_top, e.keterangan as keterangan_top');
		$this->db->from('tr_purchase_order_non_material a');
		$this->db->join('users b', 'b.id_user = a.created_by', 'left');
		$this->db->join('master_supplier c', 'c.id_suplier = a.id_suplier', 'left');
		$this->db->join('tr_top_po e', 'e.no_po = a.no_po');
		...
		foreach ($get_list_po as $item) {
			...
			$get_incoming = $this->db->get_where('warehouse_adjustment', ['no_ipp' => $item['no_po']])->result();
			$arr_id_incoming = [];
	...
	public function search_pro()
	{
		...
		foreach ($get_list_po as $item) {
			...
			$hasil .= '<td class="text-center">' . $no_purchase_invoice . '</td>';
			$hasil .= '<td class="text-center">' . $no_invoice . '</td>';
			$hasil .= '<td class="text-center">' . $no_payment . '</td>';
			$hasil .= '<td class="text-center">' . $item['nm_supplier'] . '</td>';
			$hasil .= '<td class="text-center">' . date('d F Y', strtotime($item['tanggal'])) . '</td>';
			$hasil .= '<td class="text-center">' . $item['keterangan_top'] . '</td>';
			$hasil .= '<td class="text-center">' . $sts . '</td>';
	...
	public function check_list_inc()
	{
		$get_list_inc = $this->db->query('
				...
				WHERE
					a.category = "incoming stok" OR a.category = "incoming non rutin"
			')->result_array();
	...
	public function rec_invoice_btn()
	{
		...
		$get_no_po = $this->db->query("
			...
			WHERE
				a.id IN (SELECT aa.no_ipp FROM warehouse_adjustment_detail aa WHERE aa.kode_trans IN ('" . str_replace(",", "','", implode(',', $no_incoming)) . "')) OR
		...
		$get_ttl_invoice = $this->db->query("
			...
			WHERE
				...
				c.category IN ('incoming stok', 'incoming non rutin')
			...
			FROM
				warehouse_adjustment_detail a
				JOIN tr_pr_detail_kasbon b ON IF(a.id_po_detail IS NOT NULL, b.id_detail = a.id_po_detail, b.id_detail = a.no_ipp) OR b.id_kasbon = a.no_ipp
				LEFT JOIN warehouse_adjustment c ON c.kode_trans = a.kode_trans
			WHERE
				a.kode_trans IN ('" . str_replace(",", "','", implode(',', $no_incoming)) . "') AND 
				c.category IN ('incoming stok', 'incoming non rutin')
