...
class Incoming_departemen extends Admin_Controller
{
	...
	public function index()
	{
		...
		$get_supplier = $this->db->get_where('master_supplier', ['deleted' => 0])->result_array();
		$list_supplier = [];
		foreach ($get_supplier as $item_supp) {
			$list_supplier[$item_supp['id_suplier']] = ['name_suplier' => $item_supp['name_suplier']];
		}
		...
		$this->db->join('tr_purchase_order_non_material d', 'd.no_surat = a.no_ipp', 'left');
	...
	public function process_incoming()
	{
		...
		if ($adjustment == 'IN') {
			...
			foreach ($addInMat as $val => $valx) {
				...
				// if ($cek_type == 'POX') {
				// 	$result_det	= $this->db->select('qty_in_inc as qty_in')->get_where('dt_trans_po_non_material', array('id' => $valx['id']))->result_array();
				// } else {
	...
	public function get_list_po_depart()
	{
		...
		if ($id_supplier !== '' && $id_supplier !== 'NON-PO') {
			...
			$this->db->select('a.no_po as id_po, a.no_surat as no_po, "PO" as tipe_po, a.note as keterangan, b.name_suplier as nm_supplier');
		...
		foreach ($no_po as $item_po) {
			...
			if ($item_po['nm_supplier'] == '(NON-PO)') {
				...
				$this->db->select('IF(SUM(b.qty) IS NULL, 0, SUM(b.qty)) as qty_actual');
				$this->db->from('tr_purchase_order_non_material a');
				$this->db->join('dt_trans_po_non_material b', 'b.no_po = a.no_po', 'left');
