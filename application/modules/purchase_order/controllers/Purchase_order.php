...
class Purchase_order extends Admin_Controller
{
	...
	function AddMaterial_Direct()
	{
		...
		foreach ($material as $key => $value) {
			...
			$LIST .= 	"<td>" . $value['nama_material'] . "
								...
								<input type='hidden' class='form-control input-sm' id='dt_qty_" . $key . "' name='dt[" . $key . "][qty]' value='" . $value['totalweight'] . "'>
	...
	public function getPR()
	{
		...
		foreach ($data as $key => $value) {
			...
			$option .= "<option value='" . $value['no_pr'] . "' " . $sel . ">" . $value['no_surat'] . "</option>";
		}
	...
	{
		$no_surat = $this->Pr_model->BuatNomorNew();
		print_r($no_surat);
	}
	...
	public function Request()
	{
		...
		$aktif = 'active';
		$deleted = '0';
		$po = $data = $this->db->query("SELECT * FROM tr_purchase_order WHERE status='2' AND no_po='$id' ")->result();
