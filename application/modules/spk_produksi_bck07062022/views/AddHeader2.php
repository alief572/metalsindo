<?php
	...
	$view = $results['view'];
	$nomorlot = $this->db->query("SELECT lotno FROM stock_material WHERE id_stock='$id_stock' ")->row();
?>
 ...
 <div class="box box-primary">
    <div class="box-body">
		<form id="data-form" method="post">
			<div class="col-sm-12">
				<div class="row">
					<center><label for="customer" ><h3>SPK PRODUKSI BOOKING</h3></label></center>
					<div class="col-sm-12">
						<div class="col-sm-6">
										...
										<select id="id_material" name="id_material" class="form-control select" onchange="Caristock()" required>
											<option value="">--Pilih--</option>
												<?php foreach ($results['material'] as $material){
													$selx = ($id_stock == $material->id_stock)?'selected':'';
													?>
											<option value="<?= $material->id_stock?>" <?=$selx;?>><?= strtoupper(strtolower($material->nama_material))?> - <?= ucfirst(strtolower($material->thickness))?></option>
												<?php } ?>
										</select>
										<?php } else { ?>
											<input type='text' class='form-control' id='namamaterial' onkeyup required name='namamaterial' readonly value='<?=$nama_material;?>' >
						...
						<div class="col-sm-6">
							<div class="form-group row">
								<div class="col-md-4">
									...
									<div class="col-md-8"  <?php if($view=""){ ?> id='lotnumbe_slot' <?php } ?>>
							...
							<div class="form-group row" >
								<table class='table table-bordered table-striped'>
									...
									foreach ($results['detail'] as $dt_spk){$loop++; 
										...
										$surat = $this->db->query("SELECT a.id_dt_spkmarketing, b.no_surat FROM dt_spkmarketing a 
										INNER JOIN tr_spk_marketing b ON a.id_spkmarketing = b.id_spkmarketing
										WHERE a.id_dt_spkmarketing='$dt_spk->no_surat'")->row();
										...
										echo"<th><select id='used_no_surat_$loop' name='dt[$loop][no_surat]' onchange='CariDetail($loop)' class='form-control select'>
										...
										// echo"<th>
										// <input type='text' class='form-control'  	   	value='$surat->no_surat' 	readonly id='used_nomorsuratview_$loop' required name='dt[$loop][nomorsuratview]'>
										// </th>";
										// }
										
										echo"<th id='idcust_$loop' hidden><input type='text' class='form-control' 	value='$dt_spk->idcustomer' 	readonly id='used_idcustomer_$loop' required name='dt[$loop][idcustomer]'></th>
...
<script type="text/javascript">
	...
	$(document).ready(function(){	
		...
		if(id_material != '0' && id_material != ''){
			$.ajax({
				type:"GET",
				url:siteurl+'spk_produksi/FindingStockBook2',
			...
			swal({
				...
				function(isConfirm) {
				  if (isConfirm) {
						...
						var baseurl=siteurl+'spk_produksi/SaveNewHeader2';
						$.ajax({
							url			: baseurl,
							type		: "POST",
							data		: formData,
	...
	function GetSpk(){ 
		...
		$.ajax({
            type:"GET",
            url:siteurl+'spk_produksi/GetSpkBooking',
	...
	function Caristock(){
		...
		$.ajax({
            type:"GET",
            url:siteurl+'spk_produksi/FindingStockBook',
            data:"id_material="+id_material,
            success:function(html){
               $("#lotnumbe_slot").html(html);
			   $('.select').select2();
