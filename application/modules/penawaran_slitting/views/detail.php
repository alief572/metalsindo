...
<div class="box box-primary">
	<div class="box-body">
		...
		<?php endif; ?>
		<a class="btn btn-primary btn-sm" href="<?= base_url('/penawaran_slitting') ?>" title="Detail">Back</a>
		</a>
		...
		<table id="example1" class="table table-bordered table-striped">
			...
			<tbody>
				<?php if (empty($results['detail'])) {
					...
					$numb = 0;
					foreach ($results['detail'] as $record) {
						$numb++; ?>
						<tr>
							<td><?= $numb; ?></td>
...
<script type="text/javascript">
	$(document).on('click', '.edit', function(e) {
		...
		$.ajax({
			type: 'POST',
			url: siteurl + 'penawaran_slitting/editpenawaran_slitting/' + id,
	...
	$(document).on('click', '.add', function() {
		...
		$.ajax({
			type: 'POST',
			url: siteurl + 'penawaran_slitting/addpenawaran/' + id,
			data: {
				'id': id
			},
			success: function(data) {
	...
	$(document).on('click', '.view', function() {
		...
		$.ajax({
			type: 'POST',
			url: siteurl + 'penawaran_slitting/viewpenawaran_slitting/' + id,
			data: {
				'id': id
			},
			success: function(data) {
	...
	$(document).on('click', '.delete', function(e) {
		...
		swal({
			...
			function() {
				$.ajax({
					type: 'POST',
					url: siteurl + 'penawaran_slitting/deletePenawaran',
					dataType: "json",
					data: {
						'id': id
					},
