...
class Expense extends Admin_Controller
{
	...
	public function get_kasbon($nama = '', $departement = '')
	{
		...
		$data = $this->db->query("SELECT * FROM tr_kasbon WHERE status = 3 OR id_pr IS NOT NULL")->result();
		...
		// $this->db->where('a.departement', $departement);
		// $this->db->where('a.status', 3);
		// $this->db->where('(SELECT COUNT(aa.id) FROM tr_expense_detail aa WHERE aa.id_kasbon = a.no_doc) <=', 0);
	...
	public function save()
	{
		...
		if ($id != "") {
			...
			// $result = $this->All_model->dataUpdate('tr_expense', $data, array('id' => $id));
