...
class Expense extends Admin_Controller
{
		...
		if ($id !== "") {
			...
			if (!empty($no_pr)) {
				if ($tipe_pr == 'pr departemen') {
					$get_detail_pr = $this->db->get_where('non_rutin_planning_detail', ['no_pr' => $no_pr])->result_array();
					$arrInsertDetail = [];
					$arrUpdateDetail = [];
					foreach ($get_detail_pr as $detail_pr) :
						if (isset($_POST['price_input_' . $detail_pr['id']])) {
							...
							$update_non_rutin_planning_detail = $this->db->update('non_rutin_planning_detail', ['kasbon_created' => 1], ['id' => $detail_pr['id']]);
							if (!$update_non_rutin_planning_detail) {
			...
			if ($tipe_pr !== '') {
				if ($tipe_pr == 'pr departemen') {
					...
					$this->db->update_batch('non_rutin_planning_detail', $arrUpdateDetail, 'id');
				} else if ($tipe_pr == 'pr asset') {
					$this->db->select('a.*, b.nama_asset as nm_barang, "Pcs" as satuan, b.qty');
					$this->db->from('tran_pr_header a');
					$this->db->join('asset_planning b', 'b.no_pr = a.no_pr', 'left');
	...
	public function kasbon_print($id)
	{
		...
		if ($results->tipe_pr !== '') {
			if ($results->tipe_pr == 'pr departemen') {
				...
				$this->db->from('non_rutin_planning_header a');
	...
	public function get_dat_list_kasbon()
	{
		...
		if ($this->auth->user_id() !== '1' && $this->auth->user_id() !== '202') {
