...
class Jurnal_fn extends CI_Controller {
    ...
    function jurnal_penerimaan() {
        ...
        $idcust  = $id_cust->id_customer;
                    ...
                    $No_Inv  = $kd_bayar;
                     ...
                     $deposit         = $data_bayar->tambah_lebih_bayar;	
     ...
     public function save_jurnal_bum(){
		...
		public function save_jurnal_buk_um(){
        $post        =$this->input->post();
        $session = $this->session->userdata('app_session');
		$data_session	= $this->session->userdata;
       ...
       for($i=0;$i < count($this->input->post('type'));$i++){
		...
		$jurnal_po	 = "UPDATE tr_request_payment_uangmuka SET status_jurnal='1' WHERE kd_pembayaran = '$reff' ";
     ...
     public function save_jurnal_buk_pib(){
        $post        =$this->input->post();
        $session = $this->session->userdata('app_session');
		$data_session	= $this->session->userdata;
       ...
       for($i=0;$i < count($this->input->post('type'));$i++){
		...
		$jurnal_po	 = "UPDATE tr_pib_payment SET status_jurnal='1' WHERE kd_pembayaran = '$reff' ";
        $this->db->query($jurnal_po);
    ...
    $this->db->insert(DBACC.'.javh',$dataJVhead);
	 ...
	 function jurnal_pembayaran() {
        ...
        //TERJADI SAAT CREATE PEMBAYARAN HUTANG
         ...
         $kd_bank 	        = $data_bayar->kd_bank;
         $hutangidr 	    = $data_bayar->jumlah_hutang_idr;
		 $bayaridr 	        = $data_bayar->jumlah_pembayaran_idr;
		 $selisih 	        = $data_bayar->selisih;
         $idsupplier	    = $data_bayar->supplier;
		 $nama	            = $data_bayar->nama_supplier;
	...
	function jurnal_pembayaran_um() {
        ...
        //TERJADI SAAT CREATE PENERIMAAN
         ...
         $data_bayar =  $this->db->query("SELECT * FROM tr_request_payment_uangmuka WHERE kd_pembayaran = '$kd_bayar' ")->row();

         $tgl_byr 	        = $data_bayar->tgl_pembayaran;
         $kd_invoice    	= $data_bayar->no_invoice; 
         $kd_bank 	        = $data_bayar->kd_bank;
         $sup	            = $data_bayar->id_customer;
		 $no_request		= $data_bayar->no_po;
        
         
        $id_cust =  $this->db->query("SELECT * FROM master_supplier WHERE id_suplier = '$sup'")->row();
        $idcust  = $id_cust->id_suplier;
		$nama    = $id_cust->name_suplier;
                    ...
                    $No_Inv  = $kd_bayar;
                     ...
                     $keterangan_byr  = $data_bayar->keterangan; 
                     $jumlah_terima   = $data_bayar->jumlah_bank_idr;
					 $jmlum             =$data_bayar->jumlah_piutang_idr;
					 $jmlppn            =$data_bayar->biaya_ppn_idr;					 
                     ...
                     $no_reff         = $no_request;
                         ...
                         $Keterangan_INV		    = 'Pembayaran Uang Muka Pembelian A/N '.$nama.' Kode Pembayaran. '.$No_Inv.','.$keterangan_byr;
	 ...
	 function jurnal_pembayaran_pib() {
        //JURNAL PENERIMAAN
        //TERJADI SAAT CREATE PENERIMAAN
         ...
         $data_bayar =  $this->db->query("SELECT * FROM tr_pib_payment WHERE kd_pembayaran = '$kd_bayar' ")->row();

         $tgl_byr 	        = $data_bayar->tgl_pembayaran;
         $kd_invoice    	= $data_bayar->no_invoice; 
         $kd_bank 	        = $data_bayar->kd_bank;
         $sup	            = $data_bayar->id_suplier;
		 $no_request		= $data_bayar->kd_pembayaran;
		...
		$nama    = $id_cust->name_suplier;
                    ...
                    $No_Inv  = $kd_bayar;
                     ...
                     $no_reff         = $no_request;
                         ...
                         $Keterangan_INV		    = 'Pembayaran PIB A/N '.$nama.' Kode Pembayaran. '.$No_Inv.','.$keterangan_byr;
	 ...
	 function jurnal_unbilled() {
       //JURNAL PENGAKUAN UNBILLED
       //TERJADI SAAT CREATE INVOICE
		...
		$invoicing      = $this->db->query("SELECT a.*, b.name_suplier as name_suplier FROM tr_incoming as a INNER JOIN master_supplier as b on a.id_suplier=b.id_suplier WHERE a.id_data='$id'")->row();

        $idsupp   = $invoicing->id_suplier;
		$nama   = $invoicing->name_suplier;
        $nomor  = $invoicing->id_incoming;
		$selisih  = $invoicing->selisih_kurs;
		$uangmuka  = $invoicing->uang_muka_idr;
		...
		$Keterangan_INV		    = 'Jurnal Incoming '.($nama).' No '.($nomor);
		...
		foreach($datajurnal AS $record){
			...
			if ($posisi=='D'){
			   ...
			   if($selisih < 1) {
				   ...
				   $selisihval = $selisih*(-1);
			   
			   $det_Jurnaltes[]  = array(
				  'nomor'         => '',
				  'tanggal'       => $tgl_voucher,
				  'tipe'          => 'JV',
				  'no_perkiraan'  => '7101-01-01',
				  'keterangan'    => $Keterangan_INV,
				  'no_reff'       => $id,
				  'debet'         => 0,
				  'kredit'        => $selisihval,
    ...
    public function save_jurnal_unbilled(){
    ...
    $this->db->insert(DBACC.'.javh',$dataJVhead);
     ...
     for($i=0;$i < count($this->input->post('type'));$i++){
        ...
        $Qry_Update_Cabang_acc	 = "UPDATE ".DBACC.".pastibisa_tb_cabang SET nomorJC=nomorJC + 1 WHERE nocab='101'";
        $this->db->query($Qry_Update_Cabang_acc);       
	    $jurnal_inv	 = "UPDATE tr_incoming SET status_jurnal='1' WHERE id_data = '$reff' ";
	    $this->db->query($jurnal_inv);
	...
	function jurnal_hutang_incoming() {
       ...
       //TERJADI SAAT CREATE INVOICE
        ...
        $invoicing      = $this->db->query("SELECT a.*, b.name_suplier FROM tr_request a         
        INNER JOIN master_supplier b ON a.id_suplier = b.id_suplier WHERE no_request='$id'")->row();

        $nama   = $invoicing->name_suplier;
        $nomor  = $invoicing->no_po;
		$ppn    = $invoicing->nilai_ppn;
